...
현재 저장소는 중간보고서 제출 이후 GitHub에 올려도 어색하지 않도록 구성한 **mock 데이터 기반 MVP 초안**입니다. 화면 흐름, 상태 관리 구조, repository abstraction, 주요 문서화를 먼저 정리했고, 실제 Firebase 연동 및 디바이스 검증은 이후 단계에서 진행할 예정입니다.
